...
void USART_Init(USART_TypeDef *USARTx, uint32_t  baud_rate, uint8_t parity, uint8_t stop_bits){
	...
	volatile uint32_t tmp  = USARTx->SR;
	tmp = USARTx->DR;
	(void)tmp;
	...
	uint32_t pclk = (USARTx == USART1) ? RCC_GetPclk2_Hz() : RCC_GetPclk1_Hz();
